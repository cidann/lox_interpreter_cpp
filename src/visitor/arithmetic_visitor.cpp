...
auto ArithmeticVisitor::Visit(BinaryExpression &exp)const->LoxLiterals{
    auto left=std::get<double>(exp.left_->Accept(*this));
    auto right=std::get<double>(exp.right_->Accept(*this));
...
auto ArithmeticVisitor::Visit(GroupingExpression &exp)const->LoxLiterals{
    return exp.expression_->Accept(*this);
}


auto ArithmeticVisitor::Visit(LiteralExpression &exp)const->LoxLiterals{
    return exp.value_->literal_.ToDouble();
}


auto ArithmeticVisitor::Visit(UnaryExpression &exp)const->LoxLiterals{
    auto right=std::get<double>(exp.right_->Accept(*this));
